...
export default (app: Router) => {
  ...
  route.post('/getinventory',
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const { returncode, message, data } = await inventoryServiceInstance.GetInventories(req.body as IInventory);
  ...
  route.post('/addinventory',
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const inventoryServiceInstance = Container.get(InventoryService);
        const { returncode, message, data } = await inventoryServiceInstance.AddInventory(req.body as IInventory,io);
        return res.status(200).json({ returncode, message, data });
  ...
  route.post('/editinventory',
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const inventoryServiceInstance = Container.get(InventoryService);
        const { returncode, message, data } = await inventoryServiceInstance.EditInventory(req.body as IInventory,io);
        return res.status(200).json({ returncode, message, data });
  ...
  route.post('/deleteinventory',
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const inventoryServiceInstance = Container.get(InventoryService);
        const { returncode, message, error, success  } = await inventoryServiceInstance.DeleteInventories(req.body as IInventory,io);
