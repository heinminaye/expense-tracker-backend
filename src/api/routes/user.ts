...
var UpdatePasswordSchema = Joi.object().keys({
  user_id: Joi.string().required(),
...
export default (app: Router) => {
  ...
  route.post(
    "/changepassword",
    middlewares.validation(UpdatePasswordSchema),
    middlewares.isAuth,
    middlewares.tokenCheck,
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const { returncode, message } =
          await authServiceInstance.UpdatePassword(req.body as UpdatePassword);
